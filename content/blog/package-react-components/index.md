...
description:
  ...
  We explore how to package and bundle the code with Webpack so that it can be consumed appropriately,
  and we explore how to publish and distribute it to NPM so that it can be easily accessible either
  via the public NPM repository or a private repository.
  We also analyse different theming strategies for larger Component Libraries and UI Frameworks.
...
> Check [TLDR](#tldr-how-to-package-and-publish-front-end-libraries-in-2020) if you don't want all
...
it also apply to Open Source software like a single Component package such as a Date Picker
or a Multi Select and also to Component Libraries or UI Frameworks like [React Bootstrap][9],
...
each with it's own trade-offs. Analysing each alternative is the main purpose of this post.

Unfortunately in early 2020 there still no universal standard way of Packaging a Front End module with all
...
each technique and providing example real world production ready libraries that
...
different **Theming** schemes and, at the end, recommend one.
...
CSS can be inlined without any third party library but to enable all regular CSS features
a third party **CSS-in-JS** library is recommended such as [Emotion][5] or [Styled-components][6], more on this later.
Images can be inlined by Webpack's [url-loader][11] in the form of [Data URL][100] with base 64 encoding.
...
This makes _very_ easy to package and publish to [NPM][102] and it makes very easy
...
Images are a bit more problematic because they are inlined in [base 64][103] making them 30% bigger.

In my experience, most Component Libraries do not tend to use an over abundance
of images and if they do they tend to be rather small.

Most high resolution images are dynamic content that are usually dealt by
